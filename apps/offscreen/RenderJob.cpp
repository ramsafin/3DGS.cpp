...
RenderJob loadRenderJob(
    const std::filesystem::path& configPath,
    std::optional<std::filesystem::path> outputDirectoryOverride,
    std::optional<uint8_t> physicalDeviceOverride,
    ...
    job.renderer
        .projection = {render.value("fov_degrees", 45.0f), render.value("near", 0.2f), render.value("far", 1000.0f)};
    ...
    for (size_t index = 0; index < frames.size(); ++index) {
        ...
        renderFrame.projection = {
            frame.value("fov_degrees", job.renderer.projection.horizontalFovDegrees),
