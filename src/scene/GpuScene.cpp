...
GpuScene::GpuScene(GaussianSceneData sceneData)
    : sceneData(std::move(sceneData)) {
}

void GpuScene::upload(const std::shared_ptr<VulkanContext>& context) {
    const auto vertexBytes = vkgs::core::checkedMultiply(
        sceneData.vertices.size(),
        sizeof(vkgs::render::SceneVertex),
...
void GpuScene::precomputeCov3D(const std::shared_ptr<VulkanContext>& context) {
    cov3DBuffer = createBuffer(
        context,
        vkgs::core::checkedMultiply(
            sceneData.vertices.size(),
            sizeof(vkgs::render::Cov3DUpperRight),
            "Cov3D buffer size"
        )
    );
    ...
    auto pipeline = std::make_shared<ComputePipeline>(
        context,
        std::make_shared<Shader>(context, "precomp_cov3d", SPV_PRECOMP_COV3D, SPV_PRECOMP_COV3D_len)
    );
    ...
    commandBuffer->pushConstants(
        pipeline->pipelineLayout.get(),
        vk::ShaderStageFlagBits::eCompute,
        0,
        sizeof(float),
        &scaleFactor
    );
    const auto numGroups = static_cast<uint32_t>(
        sceneData.vertices.size() / gpu::WorkgroupSize + (sceneData.vertices.size() % gpu::WorkgroupSize != 0)
    );
