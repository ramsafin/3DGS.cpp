...
class Shader {
  public:
    Shader(const std::shared_ptr<VulkanContext>& _context, std::string filename)
        : context(_context), filename(std::move(filename)) {}
    ...
    Shader(const std::shared_ptr<VulkanContext>& context, const std::string& filename, const unsigned char* data,
           size_t size)
        : filename(filename), context(context), data(data), size(size) {}
